...
var _ = Describe(`ConfigurationAggregatorV1 Examples Tests`, func() {
	...
	Describe(`ConfigurationAggregatorV1 request examples`, func() {
		...
		It(`ListConfigsNlp request example`, func() {
			fmt.Println("\nListConfigsNlp() result:")
			// begin-list_configs_nlp

			listConfigsNlpOptions := configurationAggregatorService.NewListConfigsNlpOptions(
				"How many COS buckets are there?",
				"12345",
			)
			...
			b, _ := json.MarshalIndent(listNlpResourceResponse, "", "  ")
			fmt.Println(string(b))

			// end-list_configs_nlp
