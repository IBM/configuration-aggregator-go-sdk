...
// ListConfigsNlp : Process and respond to the user's query with relevant information using large language model
// Retrieve config details based on the intent from user's query and generate response using large language model.
func (configurationAggregator *ConfigurationAggregatorV1) ListConfigsNlp(listConfigsNlpOptions *ListConfigsNlpOptions) (result *ListNLPResourceResponse, response *core.DetailedResponse, err error) {
	result, response, err = configurationAggregator.ListConfigsNlpWithContext(context.Background(), listConfigsNlpOptions)
	err = core.RepurposeSDKProblem(err, "")
	return
}

// ListConfigsNlpWithContext is an alternate form of the ListConfigsNlp method which supports a Context parameter
func (configurationAggregator *ConfigurationAggregatorV1) ListConfigsNlpWithContext(ctx context.Context, listConfigsNlpOptions *ListConfigsNlpOptions) (result *ListNLPResourceResponse, response *core.DetailedResponse, err error) {
	err = core.ValidateNotNil(listConfigsNlpOptions, "listConfigsNlpOptions cannot be nil")
	if err != nil {
		err = core.SDKErrorf(err, "", "unexpected-nil-param", common.GetComponentInfo())
		return
	}
	err = core.ValidateStruct(listConfigsNlpOptions, "listConfigsNlpOptions")
	if err != nil {
		err = core.SDKErrorf(err, "", "struct-validation-error", common.GetComponentInfo())
		return
	}
	...
	builder = builder.WithContext(ctx)
	builder.EnableGzipCompression = configurationAggregator.GetEnableGzipCompression()
	_, err = builder.ResolveRequestURL(configurationAggregator.Service.Options.URL, `/nlp_resource_query`, nil)
	if err != nil {
		err = core.SDKErrorf(err, "", "url-resolve-error", common.GetComponentInfo())
		return
	}

	sdkHeaders := common.GetSdkHeaders("configuration_aggregator", "V1", "ListConfigsNlp")
	for headerName, headerValue := range sdkHeaders {
		builder.AddHeader(headerName, headerValue)
	}

	for headerName, headerValue := range listConfigsNlpOptions.Headers {
		builder.AddHeader(headerName, headerValue)
	}
	builder.AddHeader("Accept", "application/json")
	builder.AddHeader("Content-Type", "application/json")

	if listConfigsNlpOptions.Limit != nil {
		builder.AddQuery("limit", fmt.Sprint(*listConfigsNlpOptions.Limit))
	}
	if listConfigsNlpOptions.Start != nil {
		builder.AddQuery("start", fmt.Sprint(*listConfigsNlpOptions.Start))
	}

	body := make(map[string]interface{})
	if listConfigsNlpOptions.ResourceQuery != nil {
		body["resource_query"] = listConfigsNlpOptions.ResourceQuery
	}
	if listConfigsNlpOptions.SubAccount != nil {
		body["sub_account"] = listConfigsNlpOptions.SubAccount
	}
	_, err = builder.SetBodyContentJSON(body)
	if err != nil {
		err = core.SDKErrorf(err, "", "set-json-body-error", common.GetComponentInfo())
	...
	response, err = configurationAggregator.Service.Request(request, &rawResponse)
	if err != nil {
		core.EnrichHTTPProblem(err, "list_configs_nlp", getServiceComponentInfo())
		err = core.SDKErrorf(err, "", "http-request-err", common.GetComponentInfo())
		return
	}
	if rawResponse != nil {
		err = core.UnmarshalModel(rawResponse, "", &result, UnmarshalListNLPResourceResponse)
		if err != nil {
			err = core.SDKErrorf(err, "", "unmarshal-resp-error", common.GetComponentInfo())
			return
		}
...
// ListConfigsNlpOptions : The ListConfigsNlp options.
type ListConfigsNlpOptions struct {
	// The natural language based resource query string.
	ResourceQuery *string `json:"resource_query" validate:"required"`
...
// NewListConfigsNlpOptions : Instantiate ListConfigsNlpOptions
func (*ConfigurationAggregatorV1) NewListConfigsNlpOptions(resourceQuery string, subAccount string) *ListConfigsNlpOptions {
	return &ListConfigsNlpOptions{
		ResourceQuery: core.StringPtr(resourceQuery),
		SubAccount:    core.StringPtr(subAccount),
	}
}

// SetResourceQuery : Allow user to set ResourceQuery
func (_options *ListConfigsNlpOptions) SetResourceQuery(resourceQuery string) *ListConfigsNlpOptions {
	_options.ResourceQuery = core.StringPtr(resourceQuery)
	return _options
}

// SetSubAccount : Allow user to set SubAccount
func (_options *ListConfigsNlpOptions) SetSubAccount(subAccount string) *ListConfigsNlpOptions {
	_options.SubAccount = core.StringPtr(subAccount)
	return _options
}

// SetLimit : Allow user to set Limit
func (_options *ListConfigsNlpOptions) SetLimit(limit int64) *ListConfigsNlpOptions {
	_options.Limit = core.Int64Ptr(limit)
	return _options
}

// SetStart : Allow user to set Start
func (_options *ListConfigsNlpOptions) SetStart(start string) *ListConfigsNlpOptions {
	_options.Start = core.StringPtr(start)
	return _options
}

// SetHeaders : Allow user to set Headers
func (options *ListConfigsNlpOptions) SetHeaders(param map[string]string) *ListConfigsNlpOptions {
	options.Headers = param
	return options
}
...
// ListNLPResourceResponse : The resource query response.
type ListNLPResourceResponse struct {
	// The natural language based resource query string.
	ResourceQuery *string `json:"resource_query,omitempty"`

	// The response message generated for the resource query.
	...
	// The resource details retrieved as a response to the query.
	Configs []map[string]interface{} `json:"configs,omitempty"`

	// Any additional data related to the response.
	Remarks *string `json:"remarks,omitempty"`

	// The total number of LLM tokens used during the query.
	LlmTokensUsed *int64 `json:"llm_tokens_used,omitempty"`

	// A score between 0.0 and 1.0 indicating the system's confidence in the accuracy or completeness of the resource.
	ResponseScore *float32 `json:"response_score,omitempty"`
}

// UnmarshalListNLPResourceResponse unmarshals an instance of ListNLPResourceResponse from the specified map of raw messages.
func UnmarshalListNLPResourceResponse(m map[string]json.RawMessage, result interface{}) (err error) {
	obj := new(ListNLPResourceResponse)
	err = core.UnmarshalPrimitive(m, "resource_query", &obj.ResourceQuery)
	if err != nil {
		err = core.SDKErrorf(err, "", "resource_query-error", common.GetComponentInfo())
		return
	}
	err = core.UnmarshalPrimitive(m, "response_summary", &obj.ResponseSummary)
	if err != nil {
		err = core.SDKErrorf(err, "", "response_summary-error", common.GetComponentInfo())
		return
	}
	err = core.UnmarshalPrimitive(m, "total_count", &obj.TotalCount)
	if err != nil {
	...
	err = core.UnmarshalPrimitive(m, "configs", &obj.Configs)
	if err != nil {
		err = core.SDKErrorf(err, "", "configs-error", common.GetComponentInfo())
		return
	}
	err = core.UnmarshalPrimitive(m, "remarks", &obj.Remarks)
	if err != nil {
		err = core.SDKErrorf(err, "", "remarks-error", common.GetComponentInfo())
		return
	}
	err = core.UnmarshalPrimitive(m, "llm_tokens_used", &obj.LlmTokensUsed)
	if err != nil {
		err = core.SDKErrorf(err, "", "llm_tokens_used-error", common.GetComponentInfo())
		return
	}
	err = core.UnmarshalPrimitive(m, "response_score", &obj.ResponseScore)
	if err != nil {
		err = core.SDKErrorf(err, "", "response_score-error", common.GetComponentInfo())
		return
	}
	reflect.ValueOf(result).Elem().Set(reflect.ValueOf(obj))
	return
