...
var _ = Describe(`ConfigurationAggregatorV1 Integration Tests`, func() {
	...
	Describe(`ListConfigsNlp - Process and respond to the user's query with relevant information using large language model`, func() {
		BeforeEach(func() {
			shouldSkipTest()
		})
		It(`ListConfigsNlp(listConfigsNlpOptions *ListConfigsNlpOptions)`, func() {
			listConfigsNlpOptions := &configurationaggregatorv1.ListConfigsNlpOptions{
				ResourceQuery: core.StringPtr("How many COS buckets are there?"),
				SubAccount:    core.StringPtr("12345"),
				Limit:         core.Int64Ptr(int64(38)),
				Start:         core.StringPtr("testString"),
			}

			listNlpResourceResponse, response, err := configurationAggregatorService.ListConfigsNlp(listConfigsNlpOptions)
			Expect(err).To(BeNil())
			Expect(response.StatusCode).To(Equal(200))
			Expect(listNlpResourceResponse).ToNot(BeNil())
		})
	})
